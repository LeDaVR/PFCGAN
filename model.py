...
class PIIGANModel(Model):
    ...
    def build_gan(self, x_masked, z_latent,  reuse = False, training = True):
        cnum = 32
        with tf.variable_scope("gan_fcn", reuse=reuse), arg_scope([gen_conv], padding='SAME'):
            ...
            x = tf.layers.conv2d(x, cnum, 3, 1, name='conv1', padding='SAME')	
            x = c1 = tf.layers.conv2d(x, cnum,3, 1,  name='conv2', padding='SAME') # 128
            x = tf.layers.conv2d(x, cnum *2,3, 2,  name='conv3', padding='SAME')# 64)# 64
            x = c2 = tf.layers.conv2d(x, cnum *2,3, 1,  name='conv4', padding='SAME')# 64) 
            x = tf.layers.conv2d(x, cnum *4,3, 2,  name='conv5', padding='SAME')
            x = c3 = tf.layers.conv2d(x, cnum *4,3, 1,  name='conv6', padding='SAME') # 32
            x = tf.layers.conv2d(x, cnum *8,3, 2, name='conv7', padding='SAME') # 16
            x = c4 = tf.layers.conv2d(x, cnum *8,3, 1,  name='conv8', padding='SAME')
            x = tf.layers.conv2d(x, cnum *16,3, 2,  name='conv9', padding='SAME') # 8
            x = tf.layers.conv2d(x, cnum *16,3, 1,  name='conv10', padding='SAME')
            ...
            x = tf.layers.conv2d_transpose(x, cnum * 8,3, 2,  name='deconv1', padding='SAME')
            x = tf.concat([x, c4], axis = 3, name='concat2')
            x = tf.layers.conv2d(x, cnum *8,3, 1,  name='conv11', padding='SAME')
            x = tf.layers.conv2d_transpose(x, cnum * 4,3, 2,  name='deconv2', padding='SAME')
            x = tf.concat([x, c3], axis = 3, name='concat3')
            x = tf.layers.conv2d(x, cnum * 4,3, 1,  name='conv12', padding='SAME')
            x = tf.layers.conv2d_transpose(x, cnum * 2,3, 2,  name='deconv3', padding='SAME')
            x = tf.concat([x, c2], axis = 3, name='concat4')
            x = tf.layers.conv2d(x, cnum * 2,3, 1,  name='conv13', padding='SAME')
            x = tf.layers.conv2d_transpose(x, cnum,3, 2,  name='deconv4' , padding='SAME')
            x = tf.concat([x, c1], axis = 3, name='concat5')
            x = tf.layers.conv2d(x, cnum ,3, 1,  name='conv14', padding='SAME')
            x = tf.layers.conv2d(x, 3,3, 1,  activation=tf.nn.tanh, name= 'conv15', padding='SAME')
    ...
    def build_landmark_encoder(self, x, z_rgb_masked, reuse = False, training = True):
        with tf.variable_scope("landmark_encoder", reuse=reuse):
            ...
            zl = tf.layers.dense(x, 256, name = 'zl')
            zl_var = tf.layers.dense(x, 256, name = 'zl_var')
            print("landmark encoder output shape", zl.shape)
            return zl, zl_var

    def build_face_region_encoder(self, x, z_rgb_masked, reuse=False, training = True):
        with tf.variable_scope("face_mask_encoder", reuse=reuse):
            ...
            x = flatten(x, name='flatten')
            w = x.get_shape()[1]
            zf = tf.layers.dense(x, 256, name = 'zf')
            zf_var = tf.layers.dense(x, 256, name = 'zf_var')
            return zf, zf_var 

    def build_mask_decoder(self, z, w, reuse = False, training = True):
        with tf.variable_scope("face_mask_decoder", reuse=reuse):
            cnum = 32
            z_shape_down = w // 16
            c = z.shape[1] // (z_shape_down * z_shape_down)
            z = tf.reshape(z, [-1, z_shape_down, z_shape_down, c])
            z = tf.layers.conv2d_transpose(z, cnum * 8, 4, 2, name = 'deconv1', padding='SAME')
            z = tf.layers.conv2d_transpose(z, cnum * 4, 4, 2, name = 'deconv2', padding='SAME')
            z = tf.layers.conv2d_transpose(z, cnum * 2, 4, 2, name = 'deconv3', padding='SAME')
            z = tf.layers.conv2d_transpose(z, cnum, 4, 2, name = 'deconv4', padding='SAME')
            z = tf.layers.conv2d(z, 1, 3, 1, activation=tf.nn.tanh, name='conv1', padding='SAME')
            return z

    def build_face_decoder(self, z, w, reuse = False, training = True):
        with tf.variable_scope("face_decoder", reuse=reuse):
            cnum = 32
            z_shape_down = w // 16
            c = z.shape[1] // (z_shape_down * z_shape_down)
            z = tf.reshape(z, [-1, z_shape_down, z_shape_down, c])
            ...
            z = tf.layers.conv2d_transpose(z, cnum * 2, 4, 2, name = 'deconv3', padding='SAME')
            z = tf.layers.conv2d_transpose(z, cnum, 4, 2, name = 'deconv4', padding='SAME')
            z = tf.layers.conv2d(z, 3, 3, 1, activation=tf.nn.tanh, name='conv1', padding='SAME')
            return z

    def build_landmark_decoder(self, z, w, reuse = False, training = True):
        with tf.variable_scope("landmark_decoder", reuse=reuse):
            cnum = 32
            z_shape_down = w // 16
...
#         bbox = (32, 32, 64, 64)
        ...
        ce_landmarks = - ( landmark_scaled_true * tf.log(landmark_scaled_fake + 1e-8) + (1. - landmark_scaled_true) * tf.log(1. - landmark_scaled_fake + 1e-8) )
        ce_mask = - ( mask_scaled_true * tf.log(mask_scaled_fake + 1e-8) + (1. - mask_scaled_true) * tf.log(1. - mask_scaled_fake + 1e-8) )
        # calculate cross enytropy loss for mask and landmaarks, l1 loss for part
        losses['l1_loss'] = l1_alpha * tf.reduce_mean(ce_landmarks)
        losses['ae_loss'] = l1_alpha * tf.reduce_mean(ce_mask)
        losses['ae_loss'] += l1_alpha * tf.reduce_mean(tf.abs(batch_part - fake_part))
        ...
        losses['ae_loss'] = l1_alpha * tf.reduce_mean(tf.abs(batch_norm_rgb - fake_rgb) * (1. - mask))    
...
#             padding = [[0, 0], [32, 32], [32, 32], [0, 0]]
            ...
            rec_real, *_ = self.build_inpaint_net(
                batch_incomplete_rgb, z_real, config = config, reuse = tf.AUTO_REUSE, training = training,
                padding = config.PADDING)
            ...
            losses['ae_loss_z_x'] = l1_alpha * tf.reduce_mean(tf.abs(batch_pos - rec_real) * (1. - mask))
            if not config.PRETRAIN_COARSE_NETWORK:
                losses['ae_loss_z_x'] += tf.reduce_mean(tf.abs(batch_norm_rgb - z_real_batch_predicted_rgb) * (1. - mask))
